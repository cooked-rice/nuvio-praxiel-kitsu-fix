...
async function resolveAnilistId(tmdbId) {
    ...
    return {
        anilistId,
        kitsuId: mappings.kitsu_id ? String(mappings.kitsu_id) : null,
        title,
    };
}

function parseAnimeId(value) {
    const match = String(value || "").match(/^(kitsu|anilist|mal):(\d+)(?::(\d+))?$/i);
    if (!match) return null;
    return {
        provider: match[1].toLowerCase(),
        id: match[2],
        // AIOMetadata may append the provider's real episode number here.
        episodeNumber: match[3] ? Number(match[3]) : null,
    };
}

async function resolveAnimeId(provider, id) {
    const key = provider === "anilist" ? "anilist_id" : provider + "_id";
    const data = await fetchAniZip(key, id);
    if (!data || !data.mappings) throw new Error("ani.zip: no mapping for " + provider + ":" + id);
    const mappings = data.mappings;
    const titles = data.titles || {};
    return {
        anilistId: mappings.anilist_id ? String(mappings.anilist_id) : (provider === "anilist" ? String(id) : null),
        kitsuId: mappings.kitsu_id ? String(mappings.kitsu_id) : (provider === "kitsu" ? String(id) : null),
        title: titles.en || titles["x-jat"] || titles.ja || "",
    };
}
...
async function fetchTmdbEpisode(tmdbId, season, episode) {
    try {
        const url = TMDB_API_URL + "/tv/" + encodeURIComponent(tmdbId) +
...
async function resolveAbsoluteEpisode(kitsuId, tmdbEpisode, season, episode) {
    if (!kitsuId || !tmdbEpisode) return null;
    try {
        const response = await fetch(
            ANIMAP_API + "/kitsu/" + encodeURIComponent(kitsuId) + "/episodes",
            { headers: HEADERS }
        );
        if (!response.ok) return null;
        const data = await response.json();
        const targetTitle = normalizeEpisodeTitle(tmdbEpisode.name || tmdbEpisode.original_name);
        const targetAirDate = String(tmdbEpisode.air_date || "");
        const candidates = Array.isArray(data.episodes) ? data.episodes : [];
        let best = null;
        let bestScore = 0;
        ...
        for (let i = 0; i < candidates.length; i++) {
            ...
            let score = 0;
            if (targetAirDate && candidate.airdate === targetAirDate) score += 1000;
            const candidateTitle = normalizeEpisodeTitle(
                candidate.canonical_title || candidate.title_en_us || candidate.title_en_jp || candidate.title_ja_jp
            );
            if (targetTitle && candidateTitle) {
                if (targetTitle === candidateTitle) score += 800;
                else if (targetTitle.indexOf(candidateTitle) !== -1 || candidateTitle.indexOf(targetTitle) !== -1) score += 300;
            }
            // This is only a tie-breaker. The air date/title mapping above remains authoritative.
            if (Number(candidate.season_number) === Number(season) && Number(candidate.relative_number) === Number(episode)) score += 10;
            if (score > bestScore && Number(candidate.number) > 0) {
                best = candidate;
                bestScore = score;
            }
        }
        return bestScore >= 800 ? Number(best.number) : null;
    } catch (e) {
        return null;
    }
}
...
async function getStreams(tmdbId, mediaType, season, episode) {
    try {
        ...
        const requestedAnimeId = parseAnimeId(tmdbId);
        let alId = null;
        ...
        if (requestedAnimeId) {
            const info = await resolveAnimeId(requestedAnimeId.provider, requestedAnimeId.id);
            alId = info.anilistId;
            kitsuId = info.kitsuId;
            searchTitle = info.title || "";
            // Kitsu compatibility IDs already contain the provider's canonical
            // episode number. Prefer it over a UI season-relative number.
            if (mediaType === "tv" && requestedAnimeId.episodeNumber) {
                episodeNum = requestedAnimeId.episodeNumber;
            }
        } else {
            try {
        ...
        if (!searchTitle && !requestedAnimeId) {
            const meta = await fetchTmdbTitle(tmdbId, mediaType);
            searchTitle = meta.title;
            searchYear = meta.year;
        }

        // IMDb/TMDB metadata often splits a long-running anime into seasons
